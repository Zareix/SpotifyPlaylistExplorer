  ...
  useEffect(() => {
    const getPlaylistTracks = async () => {
      ...
      while (cpt < _tracks.length) {
        ...
        const result = await axios.get(requestLink, {
          headers: { Authorization: `Bearer ${token}` },
        })
        let tracksUpdated = _tracks
        for (let i = cptD; i < cptD + result.data.artists.length; i++) {
          tracksUpdated[i].track.genres = result.data.artists[i - cptD].genres
        }
          ...
          <Dropdown className="mb-2 col-12 col-sm-6">
            ...
            <Dropdown.Menu>
              {genresToSelect.map((genre, index) => {
                return (
                  <Dropdown.Item onClick={() => selectGenre(genre)} key={index}>
                    {genre[0]}
                  </Dropdown.Item>
                )
              })}
